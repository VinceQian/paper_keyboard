# l3practice_class_json.py
#
# 本节课目标：
# 1. 复习第二节课的知识点
# 2. 理解 class 是什么
# 3. 理解 self 是什么
# 4. 理解 __init__ 什么时候运行
# 5. 理解 json 文件
# 6. 理解 json 读取后得到的还是 Python 里的 dict / list
# 7. 理解为什么项目里要把代码拆到不同文件
# 8. 为后面写 session_source.py、frame_tools.py 和 replay 流程做准备
...
print("frame1 candidate：", get_candidate(frame1))
print("frame2 candidate：", get_candidate(frame2))
print("frame3 candidate：", get_candidate(frame3))
...
print("frame1 finger 1 位置：", get_finger_position_by_id(frame1, 1))
print("frame1 finger 2 位置：", get_finger_position_by_id(frame1, 2))
print("frame2 finger 2 位置：", get_finger_position_by_id(frame2, 2))
...
# open(...) 表示打开文件
# "r" 表示 read，也就是读取
# encoding="utf-8" 表示用 utf-8 编码读取中文和普通文本
...
LAYOUT_PATH = "keyboard_number_v1.json"
...
# 如果 layout 文件已经存在，可以取消下面几行的注释来测试：
#
layout = read_layout(LAYOUT_PATH)
print("layout_id：", layout["layout_id"])
print("unit：", layout["unit"])
print("keys：", layout["keys"])
print("第一个 key 的 id：", layout["keys"][0]["id"])
...
SESSION_PATH = "test_number_input_123.json"
...
# 如果 session 文件已经存在，可以取消下面几行的注释来测试：
#
session = read_session(SESSION_PATH)
print("session_id：", session["session_id"])
print("layout_id：", session["layout_id"])
print("frame 数量：", len(session["frames"]))
print("第一帧：", session["frames"][0])
print("第一帧 candidate：", session["frames"][0]["tap"]["candidate"])
...
def print_all_candidates(session):
    ...
    for frame in frames:
        ...
        print("frame_id：", frame_id, "candidate：", candidate)


# 如果 session 文件已经存在，可以取消下面几行的注释来测试：
#
session = read_session(SESSION_PATH)
print_all_candidates(session)


# ============================================================
# 14. class 版本：SessionReader
# ============================================================

# 接下来写一个简化版 SessionSource。
#
# 它的作用：
# 1. 创建对象时读取 session json
# 2. 保存整个 session
# 3. 提供几个函数，让别人可以拿到 session 里的数据
...
print("session_id：", session_reader.get_session_id())
print("layout_id：", session_reader.get_layout_id())
print("unit：", session_reader.get_unit())
print("frame 数量：", len(session_reader.get_frames()))