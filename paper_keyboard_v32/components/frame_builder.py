class FrameBuilder:
    ...
    def build_frame(self, image, frame_id, t):
        ...
        # AudioSource 不需要 hand_data。
        # DepthSource 需要用 MediaPipe 的 z 来判断是否按下。
        # 这里用 hasattr 保持兼容：有 update() 就喂给它，没有就跳过。
        if hasattr(self.tap_source, "update"):
            self.tap_source.update(hand_data)
